...
def merge(merged_pair:tuple,vocab:dict,new_token_id:int,pair_freq_table:dict,word_freq_table:dict):
    ...
    for word_encode in word_encodings:
        ...
        for merged_pair_pos in sorted(merged_positions,reverse=True):
            # 取当前pair出现的概率
            # 生成新的word encode
            new_word_encode = word_encode[:merged_pair_pos] + (new_token_id,) + word_encode[merged_pair_pos+2:]
            ...
            word_freq_table[word_encode] -= word_freq
            if word_freq_table[word_encode] <= 0:
                word_freq_table.pop(word_encode)
            # if new_token_id==363:
            #     print([vocab[idx] for idx in word_encode])
            # 减少旧的pair出现次数
            pair_freq = word_freq
            if merged_pair_pos-1 >=0:
                left_pair_ori = (vocab[word_encode[merged_pair_pos-1]],vocab[word_encode[merged_pair_pos]])
            ...
            if merged_pair_pos+2 < len(word_encode): # right_neighbor_pos+2指这个字节对后第一个字节的位置，检查是否存在这个字节
                right_pair_ori = (vocab[word_encode[merged_pair_pos+1]],vocab[word_encode[merged_pair_pos+2]])
            ...
            if merged_pair_pos > 0:
                left_pair_new = (vocab[word_encode[merged_pair_pos-1]],vocab[new_token_id])
                pair_freq_table[left_pair_new] = pair_freq_table.get(left_pair_new,0) + pair_freq
            
            if merged_pair_pos + 2 < len(word_encode):
                right_pair_new = (vocab[new_token_id],vocab[word_encode[merged_pair_pos+2]])
                pair_freq_table[right_pair_new] = pair_freq_table.get(right_pair_new,0) + pair_freq
            
            word_encode = new_word_encode # 更新word_encode以处理下一个合并位置
...
def train_bpe_chunks(chunks: str, vocab_size:int, special_tokens:list[str],
    ...
    #             pairs.append(pair)
    #             pair_freq_table[pair] += 1
    #             pair_position[pair].append((doc_idx,word_idx,i))
    #         word_to_pair.append((word_encode,pairs))
    #     word_data.append(word_to_pair)
